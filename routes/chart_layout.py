...
# ):  
#     db = get_infra_db()
#     cursor = db.charts_layout.find({"client_id": client_id, "user_id": user_id})
#     charts = []
#     for doc in cursor:
#         charts.append({
#             "id": str(doc["_id"]),
...
#     db = get_infra_db()
#     try:
#         doc = await db.charts_layout.find_one({"_id": ObjectId(chart), "client_id": client_id, "user_id": user_id})
#     except Exception:
#         raise HTTPException(status_code=400, detail="Invalid chart id")
#     if not doc:
#         raise HTTPException(status_code=404, detail="Chart not found")
...
# @router.delete(f"/{API_VERSION}/study_templates")
# async def delete_chart(
#     client_id: str = Query(..., alias="client"),
#     user_id: str = Query(..., alias="user"),
#     chart: str = Query(..., alias="chart")
...
#     db = get_infra_db()
#     try:
#         res = await db.charts_layout.delete_one({"_id": ObjectId(chart), "client_id": client_id, "user_id": user_id})
#     except Exception:
#         raise HTTPException(status_code=400, detail="Invalid chart id")
