...
# mongo_clients = {}  # Cache clients (singleton)


# def get_mongo_client(url: str, db_name: str):
#     if db_name not in mongo_clients:
#         try:
#             logger.info(f"[MongoDB] Connecting to database: {db_name}")

#             client = MongoClient(url, serverSelectionTimeoutMS=5000)
#             client.admin.command("ping")  # check connection

#             logger.info(f"✅ [MongoDB] Connection successful: {db_name}")
...
#             logger.error(f"❌ [MongoDB] Connection FAILED: {db_name}", exc_info=True)
#             raise e  # IMPORTANT: do NOT continue to return below
